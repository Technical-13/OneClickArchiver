 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * 
 * /w/api.php?action=query&prop=revisions&format=json&rvprop=size|content&rvexpandtemplates=&rvparse=&rawcontinue=&titles=
 * gives page size in ['query']['pages'][pageID]['revisions'][0]['size']
 * var request0 = {
 * 	action: "query",
 * 	titles: archive,
 * 	prop: "revisions",
 ...
 * 	var content0 = parseInt($(response0).find('rev').attr('size'), 10);
 * 	alert(content0);//This returns the page size as raw bytes.
 * });
 * 
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * 
...
if( ( $('#ca-addsection').length > 0 || ( $.inArray('Non-talk pages that are automatically signed', mw.config.get( 'wgCategories' ))  >= 0 && mw.config.get( 'wgAction' ) == 'view' ) ) && $.inArray('Pages that should not be manually archived', mw.config.get( 'wgCategories' )) === -1){
	var request0 = {
		action:"query",
	...
	$.get(mw.config.get( 'wgScriptPath' )+"/api.php", request0, function(response0){
		var content0 = $(response0).find('rev').text();
